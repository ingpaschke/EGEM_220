...
static void	store_xacc_id(INT16*);
static void	store_av_id(INT16*,int,int);
...
static void XAccInformAcc(INT16 *msg)
{
	reg XAcc *xacc;
...
static void store_av_id(INT16 *msg,int av_msgs,int va_msgs)
{
	reg XAcc *xacc=Xaccs;
	reg int id=msg[1],i;
...
static void store_xacc_id(INT16 *msg)
{
	reg XAcc *xacc;
	reg int id=msg[1],i;
...
static int PaulaSendMsg(int msg,INT16 *mbuf)
{
...
int AvSendMsg(int sendto,int msg_id,INT16 *msg)
{
	if (sendto>=0)
	{
	\tINT16 mbuf[8];
