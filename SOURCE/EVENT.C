...
static int menu_item(int cnt,MITEM *list,INT16 *msg)
{
	WIN *win=NULL;
...
void _send_puf(int buffered,int msg_id,INT16 *msg)
{
	reg XMSG *buf = buffered ? &mbuffered : &mnobuf;
	reg int tail = buf->msg_tail,head = buf->msg_head;
	reg INT16 *mbuf = (INT16 *) &buf->msg[tail];
