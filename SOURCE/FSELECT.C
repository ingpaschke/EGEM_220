...
static FONTINFO *SetFontInfo(FONTINFO *fnt)
{
	...
	{
		INT16 d[8];
		int j1,j2,w,type = fnt->type,min_ascii,max_ascii,cw1,cw2;
...
static int cdecl draw(PARMBLK *pb)
{
	...
	if (rc_intersect((GRECT *) &pb->pb_xc,&work))
	{
		...
		FONTINFO **faces,*fnt;
		reg SLINFO *sl;
		INT16 out[4],new[4],old[4];
		reg int *sz,font,x = pb->pb_x,y = pb->pb_y,start_line,lines,offset;
...
static int cdecl draw_ex(PARMBLK *pb)
{
	...
	if (rc_intersect((GRECT *) &pb->pb_xc,&work))
	{
		INT16 old[4],new[4],extent[8];
		reg int x = pb->pb_x,y = pb->pb_y+pb->pb_h,xo,yo;
...
int _FselEvent(int events,INT16 *msg,XEVENT *event)
{
	SLINFO *slider;
