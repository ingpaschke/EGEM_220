...
int search;
INT16 work_out[57],ext_work_out[57];
...
static void scale_img(int *source,int w,int h,int n_w, int n_h, int scaling)
{
	if (source!=NULL)
	{
		reg INT16 pxy[8]; reg int dark = (scaling & DARK_SCALING),vr_mode = (dark) ? 7 : 3;
		reg MFDB image;
...
{
	reg MFDB src;
	reg INT16 pxy[8],col[2]; reg int off = (big_img==FAIL) ? 1:0;
...
{
	reg INT16 pxy[6]; reg int x=pb->pb_x,y=pb->pb_y,w=pb->pb_w,h=pb->pb_h;
...
int _messag_handler(int form_do,XEVENT *event,int *obj,DIAINFO **back)
{
	...
	reg WIN *window;
	reg INT16 *mbuf = event->ev_mmgpbuf; int msg = mbuf[0];
	...
	while ((xpop=*list++)!=NULL)
	{
		...
		if (obj==pop->p_parent || obj==pop->p_button || obj==pop->p_cycle)
		{
			...
			if (select)
			{
				...
				{
					XEVENT event;
					reg INT16 *msg = event.ev_mmgpbuf;
...
static void set_work_in(int dev,INT16 *work_in)
{
	reg int i;
	*work_in++ = dev;
...
boolean open_device(int dev,int *handle,INT16 *work_out)
{
...
boolean open_work(int *handle,INT16 *work_out)
{
	INT16 work_in[11];
	*handle = grhandle;
