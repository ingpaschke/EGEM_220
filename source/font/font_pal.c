...
{
	int2str(egem_strlcpy(egem_strlcpy(text,FastFontInfo(id)->name),", "),size,0);
...
{	
	...
	if (list_cnt>0)
	{
		int2str(egem_strlcpy(egem_strlcpy(info,*GetFilename(file_list) ? GetFilename(file_list) : "New palette"),", "),list_cnt,0);
		strcat(info," entries");
	}
	else
