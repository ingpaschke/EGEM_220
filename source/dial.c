...
void dial_options(boolean round,boolean niceline,boolean xobjs_norm,boolean ret,boolean back,boolean nonsel,boolean keys,boolean mouse,boolean clipboard,short hz)
{
	...
	if (_back_win!=old_back && bevent)
	{
		...
		#endif
				wind_set_int(window->handle,WF_BEVENT,_back_win);
	}
}
...
short objc_sysvar(short ob_smode,short ob_swhich,short ob_sival1,short ob_sival2,short *ob_soval1,short *ob_soval2)
{
	...
	{
		short ov1, ov2;
		short ret = mt_objc_sysvar(ob_smode, ob_swhich, ob_sival1, ob_sival2, &ov1, &ov2, aes_global);
		if (ob_soval1)
			*ob_soval1 = ov1;
		if (ob_soval2)
			*ob_soval2 = ov2;
...
{
	short stype, sid;
	short ret = mt_appl_search(ap_smode, ap_sname, &stype, &sid, aes_global);
	*ap_stype = stype;
	*ap_sid = sid;
	return ret;
}
...
{
	short o1, o2, o3, o4;
	short ret = mt_appl_getinfo(ap_gtype, &o1, &o2, &o3, &o4, aes_global);
	*ap_gout1 = o1;
	*ap_gout2 = o2;
	*ap_gout3 = o3;
	*ap_gout4 = o4;
...
short appl_xgetinfo(short type,short *out1,short *out2,short *out3,short *out4)
{
	...
	if (has_agi<0)
	{
		if (aes_version>=0x400 || magx || winx || appl_find("?AGI\0\0\0\0")==0)
			has_agi = 1;
		else if (aes_version > 0)
			has_agi = 0;  /* only cache negative result after init_gem completes */
	}
...
boolean driver_available(short dev)
{
