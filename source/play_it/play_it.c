...
{"","","Neues Spiel","","Bild laden","","Schwierigkeitsgrad","Einfacher","","Anzahl der Vertauschungen","Schwieriger","Raster","Raster kleiner","","Rasterweite","Raster gr��er","Hilfe anzeigen","Kein Bild geladen"};
...
void ObInfo(char *txt,int i)
{
	int x,y;
	Pos(i,&x,&y,1);
	int2str(egem_strlcpy(strend(info),txt),x,1);
	int2str(egem_strlcpy(strend(info),","),y,1);
	strcat(info,")");
}
...
void MakeInfo(int ob,int s,int d)
{
	if (s>=0)
	{
		int2str(egem_strlcpy(info,"Zug: "),user,0);
		ObInfo(", Block (",s);
		if (d>=0)
			ObInfo(" nach (",d);
		else
