...
static short matchpattern(char *s,char *p)
{
	...
	while ((p_c=*p++)!='\0')
		...
		case '[':
			...
			/* After match found, skip to closing ']' */
			if (p_c != ']')
			{
				while ((p_c = *p++) != '\0' && p_c != ']');
			}
...
boolean SaveInfoFile(char *file,boolean auto_path,void *info,short len,char *id,short version)
{
	...
	if ((handle=Fcreate(path,0))>0)
	{
		...
		cnt = (short) strlen(id);
		cnt += cnt & 1;
		if (Fwrite(handle,cnt,id)!=cnt || Fwrite(handle,2,&crc)!=2 ||
			Fwrite(handle,2,&version)!=2 || Fwrite(handle,len,info)!=len)
			err++;
		if (Fclose(handle)<0)
			err++;
		if (err)
...
short LoadInfoFile(char *file,boolean auto_path,void *info,short len,short min_len,char *id,short min_version)
{
	...
	if ((!auto_path || shel_find(path)) && (handle=Fopen(path,0))>0)
	{
		...
		{
			if ((rd=Fread(handle,len,mem))>=(min_len+cnt) && rd<=len)
			{
				err = FALSE;
				if (memcmp(mem,id,cnt-4)==0)
				{
					mem += cnt;
					{
						short crc_stored, ver_stored;
						memcpy(&crc_stored, mem - 4, sizeof(short));
						memcpy(&ver_stored, mem - 2, sizeof(short));
						if (calc_crc((unsigned char *) mem, len = rd - cnt) == crc_stored && ver_stored >= min_version)
