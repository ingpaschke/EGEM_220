...
/* Not thread-safe: global arrays shared by all callers.
   GEM calls must be serialized (standard for single-threaded TOS apps). */
short _egem_intin[140];
short _egem_intout[140];
...
short __aes__(unsigned long packed)
{
    ...
    aespb[4] = _egem_addrin;
    aespb[5] = _egem_addrout;
